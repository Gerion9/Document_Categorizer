...
export const GlassButton = React.forwardRef<HTMLButtonElement, GlassButtonProps>(
  ({ className = "", variant = "secondary", children, ...props }, ref) => {
    ...
    const baseStyle = "relative inline-flex items-center justify-center px-4 py-2 text-sm font-medium transition-[background-color,border-color,color,box-shadow,opacity] duration-200 rounded-xl overflow-hidden focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-1 disabled:opacity-50 disabled:cursor-not-allowed";
    
    const filterId = "glass-button";
    const variantClass =
      variant === "primary"
        ? "text-white bg-brand-600 hover:bg-brand-500 shadow-md"
        : variant === "danger"
          ? "text-white bg-red-600 hover:bg-red-500 shadow-md"
          : variant === "ghost"
