...
Flow:
  1. BOS redirects user to the frontend with email + name + HMAC signature
  2. Frontend POSTs to /api/sso-login
  3. We verify the HMAC, create/update the user in our DB (Just-in-Time),
     and return a local JWT with user_id + permissions
  4. Frontend uses GET /api/auth/me to fetch the user profile with roles/permissions

Note: New users are auto-provisioned with default role "casemanager" on first SSO login.
"""
...
from ..models import User, UserRole, Role, RolePermission, UserPermission
...
def require_role(role_name: str):
    ...
    Usage in a router:
        supervisor_required = Depends(require_role("supervisor"))
    """
    def _check_role(
        ...
        if not has_role:
            raise HTTPException(403, f"Role '{role_name}' required")
        return payload
    return _check_role


def require_any_role(*role_names: str):
    """Factory dependency that allows access if user has any listed role."""
    ...
    def _check_roles(
        ...
        if not has_any_role:
            roles_text = ", ".join(sorted(normalized))
            raise HTTPException(403, f"One of roles [{roles_text}] required")
        return payload
...
def sso_login(data: SSOLoginRequest, db: Session = Depends(get_db)):
    ...
    if user:
        ...
        casemanager_role = db.query(Role).filter(Role.name == "casemanager").first()
        if not casemanager_role:
            raise HTTPException(
                status_code=500,
                detail="Role 'casemanager' not configured",
            )
        ...
        db.add(UserRole(user_id=user.id, role_id=casemanager_role.id))
        db.commit()
        db.refresh(user)
